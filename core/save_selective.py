...
class CoLA(nn.Module):
    ...
    def forward(self, x,train):
        ...
        #    re_a_01 = self.relation_module(easy_act_2,hard_act)
            ...
            re_a_02,_ = self.actionness_module(act_32,False)
            act_03 = torch.cat([easy_act_0,  easy_act_3], dim=1).transpose(-1, -2)
            re_a_03,_ = self.actionness_module(act_03,False)
            act_1h = torch.cat([easy_act_1,  hard_act], dim=1).transpose(-1, -2)
            re_a_0h,attention_weights = self.actionness_module(act_1h,False)
        ...
        #   re_b_01 = self.relation_module(easy_bkg_2, easy_bkg_1)
            ...
            re_b_02,_ = self.actionness_module(bkg_32, False)
            bkg_03 = torch.cat([easy_bkg_0, easy_bkg_3], dim=1).transpose(-1, -2)
            re_b_03,_ = self.actionness_module(bkg_03, False)
            bkg_1h = torch.cat([easy_bkg_1, hard_bkg], dim=1).transpose(-1, -2)
            re_b_0h,_ = self.actionness_module(bkg_1h, False)

            re_ab_23,_ = self.actionness_module(torch.cat([easy_act_2,easy_bkg_3],dim=1).transpose(-1,-2),False)
            re_ab_32,_ = self.actionness_module(torch.cat([easy_bkg_2,easy_act_3],dim=1).transpose(-1,-2),False)
            re_ab_33,_ =  self.actionness_module(torch.cat([easy_act_3,easy_bkg_3],dim=1).transpose(-1,-2),False)
            re_ab_h3,_ =   self.actionness_module(torch.cat([hard_act,easy_bkg_3],dim=1).transpose(-1,-2),False)
            re_ab_3h,_ =  self.actionness_module(torch.cat([easy_act_3,hard_bkg],dim=1).transpose(-1,-2),False)
            re_ab_hh,_ =    self.actionness_module(torch.cat([hard_bkg,hard_act],dim=1).transpose(-1,-2),False)
         ...
         #   re_b_01 = self.relation_module(easy_bkg_0, hard_bkg)
            ...
            return video_scores, actionness,actionness_2, cas,re_s_scores,re_d_scores,contrast_pairs,attention_weights
        else:
