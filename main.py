...
from core.loss import TotalLoss_17
...
def set_path(config):
    if config.MODE == 'train':
        config.EXP_NAME = 'experiments/{cfg.MODE}/etheta_{cfg.E_THETA}_theta_{cfg.THETA}_freq_{cfg.TEST_FREQ}' \
...
def train_one_step(net, loader_iter, optimizer, criterion, writter, step):
    ...
    optimizer.zero_grad()
    video_scores, actionness,actionness_2, cas,re_s_scores,re_d_scores,contrast_pairs = net(data,True)
    cost, loss = criterion(video_scores, label, contrast_pairs,re_s_scores, re_d_scores,actionness,actionness_2)
...
def test_all(net, cfg, test_loader, test_info, step, writter=None, model_file=None):
    ...
    for data, label, _, vid, vid_num_seg in test_loader:
        ...
        video_scores, actionness, cas = net(data,False)

        label_np = label.cpu().data.numpy()
        score_np = video_scores[0].cpu().data.numpy()
        #
        # np_action = actionness.data.cpu().numpy()
        # save_path = 'save_npy_bilateral/' + vid[0] + '.npy'
        # np.save(save_path, np_action)
